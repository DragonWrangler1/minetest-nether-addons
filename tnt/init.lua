...
local S = minetest.get_translator("nether_tnt")
...
function   nether_tnt.burn(pos, nodename)
...
local function tnt_explode(pos, radius, ignore_protection, ignore_on_blast, owner, explode_center)
	...
	local count = 0
	local c_tnt
	local c_tnt_burning = minetest.get_content_id("nether_tnt:tnt_burning")
	local c_tnt_boom = minetest.get_content_id("nether_tnt:boom")
	local c_air = minetest.CONTENT_AIR
	local c_ignore = minetest.CONTENT_IGNORE
        if minetest.get_modpath("fire") then
		c_tnt = minetest.get_content_id("nether_tnt:tnt")
	else
...
function nether_tnt.boom(pos, def)
	def = def or {}
	...
	if not def.explode_center and def.ignore_protection ~= true then
		minetest.set_node(pos, {name = "nether_tnt:boom"})
	end
...
minetest.register_node("nether_tnt:boom", {
	drawtype = "airlike",
...
minetest.register_node("nether_tnt:gunpowder", {
	...
	on_punch = function(pos, node, puncher)
		if puncher:get_wielded_item():get_name() == "default:torch" then
			minetest.set_node(pos, {name = "nether_tnt:gunpowder_burning"})
			default.log_player_action(puncher, "ignites nether_tnt:gunpowder at", pos)
		end
	end,
...
minetest.register_node("nether_tnt:gunpowder_burning", {
	drawtype = "raillike",
	paramtype = "light",
	sunlight_propagates = true,
	walkable = false,
...
minetest.register_craft({
	output = "nether_tnt:gunpowder 5",
	type = "shapeless",
	recipe = {"nether:nether_lump", "default:gravel"}
})

minetest.register_craftitem("nether_tnt:tnt_stick", {
...
if minetest.get_modpath("mobs_npc") then
	minetest.register_craft({
		output = "nether_tnt:tnt_stick 2",
		recipe = {
			{"nether_tnt:gunpowder", "", "nether_tnt:gunpowder"},
			{"nether_tnt:gunpowder", "default:paper", "nether_tnt:gunpowder"},
			{"nether_tnt:gunpowder", "", "nether_tnt:gunpowder"},
		}
	})

	minetest.register_craft({
		output = "nether_tnt:tnt",
		recipe = {
			{"nether_tnt:tnt_stick", "nether_tnt:tnt_stick", "nether_tnt:tnt_stick"},
			{"nether_tnt:tnt_stick", "nether_tnt:tnt_stick", "nether_tnt:tnt_stick"},
			{"nether_tnt:tnt_stick", "nether_tnt:tnt_stick", "nether_tnt:tnt_stick"}
		}
	})
	...
	minetest.register_abm({
		label = "TNT ignition",
		nodenames = {"group:tnt", "nether_tnt:gunpowder"},
		neighbors = {"fire:basic_flame", "default:lava_source", "default:lava_flowing"},
		interval = 4,
		chance = 1,
		action = function(pos, node)
			nether_tnt.burn(pos, node.name)
		end,
	})
end

function nether_tnt.register_tnt(def)
	local name
	if not def.name:find(':') then
		name = "nether_tnt:" .. def.name
	else
		name = def.name
	...
	minetest.register_node("nether_tnt:tnt_burning", {
		tiles = {
			{
